...
public class BlockCutterItems extends BaseItems {
    ...
    private static Item cutterItemHelper(String name, int maxCount) { return registerItem(name, new cutterPlans(new FabricItemSettings().maxCount(maxCount).group(Delbase.DELBASE_ITEMS))); }
    private static Item compoundItemHelper(String name, int maxCount) { return registerItem(name, new compoundPlans(new FabricItemSettings().maxCount(maxCount).group(Delbase.DELBASE_ITEMS))); }
