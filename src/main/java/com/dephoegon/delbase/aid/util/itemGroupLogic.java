...
public class itemGroupLogic {
    public static final ItemGroup DELBASE_BLOCKS = Registry.register(Registries.ITEM_GROUP,
            new Identifier(Delbase.Delbase_ID, "dephoegon_blocks"),
            FabricItemGroup.builder().displayName(Text.translatable("itemGroup.dephoegon_blocks"))
                    .icon(() -> new ItemStack(machineBlock.BLOCK_CUTTER_STATION)).entries((displayContext, entries) ->
                            getDelBlocks().forEach((entries::add))).build());
