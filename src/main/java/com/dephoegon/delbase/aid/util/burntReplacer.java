...
public class burntReplacer implements BlockOnFireCallBack {
    ...
    public static void AshBlock(World world, BlockPos blockpos, @NotNull BlockState state) {
        ...
        if (block instanceof PillarBlock) {
            BlockState targetDefaultState = getVanilla_wood_list().contains(block.getDefaultState()) ? ASH_BLOCK.getDefaultState() : ASH_LOG.getDefaultState();
            blockReplacer.AxisPlacement(world, blockpos, state, targetDefaultState);
            return;
        }
