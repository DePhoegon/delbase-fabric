...
public class slabStrippedWoods extends baseModBlocks {
    ...
    public static final SlabBlock STRIPPED_CHERRY_WOOD_SLAB = noTooltipHelper("stripped_cherry_wood_slab", CHERRY_WOOD, true);
    public static final SlabBlock STRIPPED_CHERRY_LOG_SLAB = noTooltipHelper("stripped_cherry_log_slab", CHERRY_WOOD, true);
    public static final SlabBlock STRIPPED_BAMBOO_SLAB = noTooltipHelper("stripped_bamboo_block_slab", OAK_WOOD, true);
    ...
    private static SlabBlock woodSlabHelper(String name, Block block, String NormToolTip, String ShiftToolTip, String CtrlToolTip, SlabBlock strippedSlab, int fuelTime, int burnChance, int burnSpread) {
        ...
        if (fuelTime > 0) {
            hold = (SlabBlock) registerBlock(name,
                    new strippedWoodSlab(AbstractBlock.Settings.copy(block).sounds(BlockSoundGroup.WOOD), NormToolTip, ShiftToolTip, CtrlToolTip, strippedSlab), true, fuelTime, burnChance, burnSpread);
        } else {
            hold = (SlabBlock) registerBlock(name,
                    new strippedWoodSlab(AbstractBlock.Settings.copy(block).sounds(BlockSoundGroup.WOOD), NormToolTip, ShiftToolTip, CtrlToolTip, strippedSlab)
            );
        }
        return hold;
    }
