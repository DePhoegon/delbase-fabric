    ...
    private static StairsBlock woodStairHelper(String name, Block block, String NormToolTip, String ShiftToolTip, String CtrlToolTip, StairsBlock strippedStair, int fuelTime, int burnChance, int burnSpread) {
        ...
        if (fuelTime > 0) {
            hold = (StairsBlock) registerBlock(name,
                    new woodStair(block.getDefaultState(), AbstractBlock.Settings.copy(block).sounds(BlockSoundGroup.WOOD), NormToolTip, ShiftToolTip, CtrlToolTip, strippedStair), true, fuelTime, burnChance, burnSpread);
        } else {
            hold = (StairsBlock) registerBlock(name,
                    new woodStair(block.getDefaultState(), AbstractBlock.Settings.copy(block).sounds(BlockSoundGroup.WOOD), NormToolTip, ShiftToolTip, CtrlToolTip, strippedStair)
            );
        }
        return hold;
    }
